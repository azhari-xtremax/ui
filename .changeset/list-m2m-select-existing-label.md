...
Same bug, same fix, for "Create New": `handleEditFormSuccess` already receives the full just-created record from `CollectionForm`'s `onSuccess`, but only forwarded `data.id` to `selectItems`, discarding the rest — now the whole record is passed through so the template resolves without an extra fetch.
